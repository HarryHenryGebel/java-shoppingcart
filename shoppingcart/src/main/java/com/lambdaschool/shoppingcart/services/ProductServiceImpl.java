...
public class ProductServiceImpl implements ProductService {
  ...
  public List<Product> findAll() {
     ...
     */
    productrepos.findAll().iterator().forEachRemaining(list::add);
    return list;
  }
  ...
  public Product save(Product product) {
    if (product.getCarts().size() > 0) {
      throw new ResourceFoundException(
        "Carts are not updated through Products"
      );
    }
  ...
  public Product update(long id, Product product) {
    if (product.getCarts().size() > 0) {
      throw new ResourceFoundException(
        "Carts cannot be updated through this process"
      );
    }

    Product currentProduct = productrepos
      .findById(id)
      .orElseThrow(
        () -> new ResourceNotFoundException("Product id " + id + " not found!")
      );
    ...
    productrepos.updateProductInformation(
